...
#else
int _INIT_4_SDL2MixerBase(void) __attribute__((alias("__CSTP_init_SDL2MixerBase")));
void _EXIT_4_SDL2MixerBase(void) __attribute__((alias("__DSTP_cleanup_SDL2MixerBase")));

struct Library *SDL2MixerBase;

static CONSTRUCTOR_P(init_SDL2MixerBase, 100)
{
...
static DESTRUCTOR_P(cleanup_SDL2MixerBase, 100)
{
	CloseLibrary(SDL2MixerBase);
}
#endif
